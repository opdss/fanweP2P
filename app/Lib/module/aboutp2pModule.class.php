...
class aboutp2pModule  extends SiteBaseModule {
    function index() {
		$GLOBALS['tmpl']->caching = true;
		$GLOBALS['tmpl']->cache_lifetime = 6000;  //首页缓存10分钟
		$cache_id  = md5(MODULE_NAME.ACTION_NAME);	
		if (!$GLOBALS['tmpl']->is_cached("page/aboutp2p.html", $cache_id))
		{	
			$info = get_article_buy_uname("aboutp2p");
		...
		$GLOBALS['tmpl']->display("page/aboutp2p.html",$cache_id);
    }
}
?>